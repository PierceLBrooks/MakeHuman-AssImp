...
mh::Vertex::Vertex(const float x, const float y, const float z)
{
    pos.x = x;
    pos.y = y;
    pos.z = z;
...
void mh::Vertex::sort_weight()
{
    std::sort(weights.begin(), weights.end(), greater_function);
}

bool mh::Vertex::greater_function(Weight a, Weight b)
{
    return (a.weight > b.weight);
}

mh::Triangle::Triangle(const int v0, const int v1, const int v2)
{
    v[0] = v0;
    v[1] = v1;
    v[2] = v2;
...
mh::Bone::Bone(const std::string _name,
...
mh::Bone_Point::Bone_Point(const int _index, const structvec3& _pos)
...
bool mh::PositionCompare::FloatEqual(float f1, float f2) const
{
...
bool mh::PositionCompare::operator()(const structvec3& a, const structvec3& b) const
{
...
mh::VoxelGrid::VoxelGrid(const std::string& _mesh_file,
          ...
          const bool _detect_solidify)
{
    ...
    mt_dist_00_10 = std::uniform_real_distribution<float>(0.0f, 1.0f);
    mt_dist_05_05 = std::uniform_real_distribution<float>(-0.5f, 0.5f);
    mt_dist_00_2PI = std::uniform_real_distribution<float>(0.0f, 2.0f * MH_PI);
    mt_dist_N10_10 = std::uniform_real_distribution<float>(-1.0f, 1.0f);
...
void mh::VoxelGrid::calculate_all_voxel_darkness()
{
...
void mh::VoxelGrid::diffuse_all_heats()
{
    int supported_thread_num = std::thread::hardware_concurrency();
    // at least we can do with one thread
    if (supported_thread_num == 0) {
...
int mh::VoxelGrid::get_nearest_bone(const int index)
{
...
void mh::VoxelGrid::generate_weight_for_vertices()
{
...
void mh::VoxelGrid::read_bone_from_file(const std::string& filename)
{
    std::ifstream fin;
...
bool mh::VoxelGrid::has_triangle(Voxel& voxel)
{
    return voxel.triangles.size() > 0;
}

bool mh::VoxelGrid::has_vertex(Voxel& voxel)
{
    return voxel.vertices.size() > 0;
}

void mh::VoxelGrid::get_vertex_bound_box(const Vertex& vertex,
                          structvec3&     bb_min,
                          structvec3&     bb_max)
{
...
void mh::VoxelGrid::get_triangle_bound_box(const Triangle& triangle,
                            structvec3&     bb_min,
                            structvec3&     bb_max)
{
    for (int i = 0; i < 3; i++) {
...
void mh::VoxelGrid::add_all_triangles()
{
...
void mh::VoxelGrid::add_triangle(int n)
{
    // init the bound box
    structvec3 bb_min(FLT_MAX, FLT_MAX, FLT_MAX);
    structvec3 bb_max(FLT_MIN, FLT_MIN, FLT_MIN);
...
void mh::VoxelGrid::add_all_bones()
{
...
void mh::VoxelGrid::add_bone(const std::vector<Bone>& bones, int bone_index)
{
...
void mh::VoxelGrid::shrink_all_vertex_neibours()
{
...
void mh::VoxelGrid::add_all_vertices()
{
    const float inv_grid_size = 1.0f / grid_size;
    for (int i = 0; i < static_cast<int>(vertices.size()); i++) {
...
void mh::VoxelGrid::shrink_all_vertices()
{
...
void mh::VoxelGrid::shrink_all_triangles()
{
    // Loop through the voxel grids
    for (int i = 0; i < static_cast<int>(voxels.size()); i++) {
...
void mh::VoxelGrid::init_all_vertex_heats()
{
...
void mh::VoxelGrid::bone_point_darkness(int index)
{
    const float inv_grid_size = 1.0f / grid_size;
...
void mh::VoxelGrid::bone_point_glow(int index)
{
...
void mh::VoxelGrid::calculate_darkness_in_range(const int start, const int block_size)
{
    // Loop through in the range.
...
void mh::VoxelGrid::diffuse_vertex_in_range(const int start, const int block_size)
{
    // Loop through in the range.
...
void mh::VoxelGrid::ray_cast_darkness(const structvec3& ray_origin,
...
int mh::VoxelGrid::ray_cast_triangle(const structvec3& ray_origin,
                             const structvec3& ray_dir,
                             const Voxel&      voxel,
...
void mh::VoxelGrid::search_vertices_in_range(const structvec3& pos,
                              const float radius,
...
float mh::VoxelGrid::vertex_heat_standard_error()
{
    // Loop through the vertices
...
void mh::VoxelGrid::diffuse_vertex(const int index)
{
...
void mh::VoxelGrid::split_string(const std::string&        s,
